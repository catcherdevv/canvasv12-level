    ...
    if (talkedRecently.get(message.author.id)) {
        ...
        if (level.xp >= nextXP) {
           ...
           /* let embed = new Discord.MessageEmbed()
            ...
            try {
                client.channels.cache.get('822763553119141888').send(`**Tebrikler** ${message.author}! Şimdi seviye atladınız **level ${level.level}**`)
            } catch (err) {
...
var roleLogs = "822763553119141888"

client.on("guildMemberRoleAdd", async(member, role) => {
 let entry = await member.guild.fetchAuditLogs({type: 'GUILD_MEMBER_UPDATE'}).then(audit => audit.entries.first());
if (entry.executor.id != client.user.id) return;
client.channels.cache.get(roleLogs).send(`**Tebrikler** ${member}! seviye atladın ve \`${role.name}\` rolü verildi.`);
});